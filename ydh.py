...
with col1:
    st.title("YouTube Data Exploration and Analysis")
with col2:
    st.markdown("""
     To explore and analyze data from YouTube channels and perform various data queries.
    """)
channel_id = st.text_input("Enter YouTube Channel ID:", placeholder="e.g., UC_07DJz-yVOl3pBqc-PXcMZQ")
...
if st.button("Switch to dataframe"):
    Table=tables(unique_channel)
    st.success(Table)

# Table Selection with Dropdown or Checkbox (modify based on preference)
table_options = ("CHANNELS", "PLAYLISTS", "VIDEOS", "COMMENTS")
show_table = st.selectbox("Select the Table for View", table_options)  # Dropdown option

# Alternative: Checkbox selection (uncomment if preferred)
# show_table = st.checkbox("CHANNELS"), st.checkbox("PLAYLISTS"), st.checkbox("VIDEOS"), st.checkbox("COMMENTS")

# Call functions based on selected table
if show_table:
    # Check if single selection with dropdown or multiple selection with checkbox
    if isinstance(show_table, str):  # String indicates single selection
    ...
    else:  # List indicates multiple selection (if using checkbox)
        selected_tables = [option for option, checked in zip(table_options, show_table) if checked]
...
questions=st.selectbox("Select your queries",("names of all the videos with channel names",
...
if questions=="names of all the videos with channel names":
...
elif questions=="number of videos, and how many videos they have":
...
elif questions=="top 10 most viewed videos with channel names":
...
elif questions=="comments were made on each video, with video titles":
...
elif questions=="number of likes and who got more likes":
...
# the Question is get number likes and dislikes in each videos but youtube is hided the dislikes data for community purpose .rate() is not available
elif questions=="who got likes for each video":
    ...
    st.write(df6)
elif questions=="views for each channel":
    ...
    st.write(df7) 
elif questions=="published videos in the year 2022":
    ...
    st.write(df8)
elif questions=="average duration of all videos in each channel":
    ...
    st.write(df9_1)
elif questions=="highest number of comments":
...
# UI styile
page_bg_img = f""" <style> [data-testid="stAppViewContainer"] {{ background-color: #ecc1c1; }} </style> """
